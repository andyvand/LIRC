...
static char *accept_rx_byte(unsigned char rch, struct ir_remote *remotes)
{
	...
	case RX_STATE_PSM:
		switch (rch) {
		case 0x02:
			curr_queue->msg_id = MSG_ID_PORT_DISABLED;
			break;
		case 0x03:
			curr_queue->msg_id = MSG_ID_PORT_ENABLED;
			break;
		case 0x81:
			curr_queue->msg_id = MSG_ID_TRANSMISSION_TIMEOUT;
			break;
		case 0xff:
			curr_queue->msg_id = MSG_ID_ILLEGAL_COMMAND;
			break;
		case 0x80:
			curr_queue->msg_id = MSG_ID_RECEIVE_ERROR;
			break;
		case 0x82:
			curr_queue->msg_id = MSG_ID_INVALID_SAMPLE_PERIOD;
			break;
		case 0x83:
			curr_queue->msg_id = MSG_ID_SERIAL_PORT_RECEIVE_BUFFER_OVERFLOW;
			break;
		case 0x86:
			curr_queue->msg_id = MSG_ID_SERIAL_PORT_RECEIVE_BUFFER_OVERRUN;
			break;
		case 0x85:
			curr_queue->msg_id = MSG_ID_SERIAL_PORT_RECEIVE_FRAMING_ERROR;
			break;
		case 0x84:
			curr_queue->msg_id = MSG_ID_BAUD_RATE_ILLEGAL;
			break;
		case 0x0f:
			curr_queue->msg_id = MSG_ID_DEFAULTS_LOADED;
			break;
		case 0x0d:
			curr_queue->msg_id = MSG_ID_DEFAULTS_SAVED;
			break;
		case 0x8f:
			curr_queue->msg_id = MSG_ID_SEEPROM_WRITE_ERROR;
			break;
		default:
			...
			switch (rch) {
			case 0x04:
				curr_queue->msg_id = MSG_ID_SAMPLING_PERIOD_EQUALS;
				msg_len = 2;
				break;
			case 0x06:
				curr_queue->msg_id = MSG_ID_CARRIER_PERIOD_EQUALS;
				msg_len = 2;
				break;
			case 0x0c:
				if (port_id == PORT_SYS) {
			...
			case 0x0a:
				...
				break;
			case 0x10:
				curr_queue->msg_id = MSG_ID_HANDSHAKING_MODE_EQUALS;
				break;
			case 0x12:
