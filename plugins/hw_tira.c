...
void displayonline(void)
{
	...
	if (hw.rec_mode == LIRC_MODE_LIRCCODE)
		...
		dmode = mode_timing;
	logprintf(LOG_INFO, "device online, ready to %s remote codes(%s)", dflags, dmode);
}
...
int child_process(int pipe_w, int oldprotocol)
{
	...
	while (1) {
		...
		if (!FD_ISSET(hw.fd, &read_set))
			continue;
		readsize = read(hw.fd, &tirabuffer[tirabuflen], sizeof(tirabuffer) - tirabuflen);
		if (readsize <= 0) {
...
int check_tira(void)
{
	...
	if (!tty_reset(hw.fd) || !tty_setbaud(hw.fd, 9600) || !tty_setrtscts(hw.fd, 1))
		return 0;
...
int check_ira(void)
{
	...
	if (!tty_reset(hw.fd) || !tty_setbaud(hw.fd, 9600) || !tty_setrtscts(hw.fd, 0) || !tty_setdtr(hw.fd, 1))
		return 0;
...
static int tira_send(struct ir_remote *remote, struct ir_ncode *code)
{
	...
	if (i != length)
		...
		if (strncmp((char *)wrtbuf, "OIX", 3) == 0)
			...
			logprintf(LOG_ERR, "no response from device");
	}
