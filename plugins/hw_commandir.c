...
int claim_and_setup_commandir(unsigned int bus_num, int devnum, struct usb_device *dev)
{
	...
	if (new_commandir->cmdir_udev == NULL) {
		logprintf(LOG_ERR, "Error opening commandir - bus %d, device %d.", bus_num, devnum);
		free(new_commandir);
		return FALSE;
	}
	...
	if (r < 0) {
		free(new_commandir);
		logprintf(LOG_ERR, "Unable to claim CommandIR (error %d) - Is it already busy?", r);
		logprintf(LOG_ERR, "Try 'rmmod commandir' or check for other lircds");
		return FALSE;
	}
	...
	case 2:
		...
		while (tries--) {
			...
			send_status = usb_bulk_write(new_commandir->cmdir_udev, 2,	// endpoint2
						     get_version, 2, 1500);
			if (send_status < 0) {
				logprintf(LOG_ERR, "Unable to write version request - Is CommandIR busy? Error %d",
					  send_status);
				break;
			}
			...
			if (send_status < 0) {
				logprintf(LOG_ERR, "Unable to read version request - Is CommandIR busy? Error %d",
					  send_status);
			...
			if (send_status == 3) {
				if (commandir_data_buffer[0] == GET_VERSION) {
					...
					logprintf(LOG_ERR, "Hardware revision is %d.%d.", commandir_data_buffer[1],
...
static void hardware_scan()
{
	...
	for (a = first_commandir_device; a; a = a->next_commandir_device) {
		...
		if (!found) {
			...
			logprintf(LOG_ERR, "Commandir removed from [%d][%d].", a->busnum, a->devnum);
...
static void hardware_setorder()
{
	...
	if (first_commandir_device && first_commandir_device->next_commandir_device) {
		...
		for (pcd = first_commandir_device; pcd; pcd = pcd->next_commandir_device) {
			logprintf(LOG_INFO, " CommandIR Index: %d (Type: %d, Revision: %d), Emitters #%d-%d",
			...
			if (pcd->rx_data_available) {
				...
				if (read_retval < 1) {
					if (read_retval == -19) {
						...
						logprintf(LOG_ERR, "Error %d trying to read %d bytes", read_retval,
							  pcd->endpoint_max[3]);
					}
...
static void recalc_tx_available(struct commandir_device *pcd)
{
	...
	if (pcd->lastSendSignalID != pcd->commandir_last_signal_id) {
		...
		logprintf(LOG_ERR, "Error: required the failsafe %d != %d", pcd->lastSendSignalID,
